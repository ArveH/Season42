...
**`DEVELOPMENT_TEAM` is not in the pbxproj, and should not go back in.** Xcode writes it there whenever it is given a team through the target's Signing & Capabilities pane, which puts one developer's account in everyone's checkout and defeats the split above. Put yours in `Local.xcconfig`; if a `DEVELOPMENT_TEAM = …` line shows up in a `project.pbxproj` diff, that is what happened.
