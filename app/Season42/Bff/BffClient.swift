...
struct BffClient: LogoSearching, SeriesSearching {
    ...
    func providers(matching text: String) async throws -> [WatchProvider] {
        let json = try await fetchSearch(at: "providers", for: text)
        do {
    ...
    func series(matching text: String) async throws -> [SeriesMatch] {
        let json = try await fetchSearch(at: "series", for: text)
        do {
    ...
    /// One of the BFF's searches, asked for `text`. `query` is what the BFF calls the text a
    /// search is for, so the app spells it the same way wherever it asks.
    private func fetchSearch(at endpoint: String, for text: String) async throws -> Data {
        var components = URLComponents(
            url: baseUrl.appending(path: endpoint),
            resolvingAgainstBaseURL: false
        )
        components?.queryItems = [URLQueryItem(name: "query", value: text)]
        guard let url = components?.url else { throw BffError.notReached }

        // A search asks what the BFF says *now*, so what `URLSession` happens to have kept
        // from the last one is of no use.
        return try await fetch(url, ignoringWhatWasCached: true)
    }
    ...
    private func fetch(_ url: URL, ignoringWhatWasCached: Bool = false) async throws -> Data {
        ...
        guard let answer = response as? HTTPURLResponse else { throw BffError.notReached }
        guard answer.statusCode == 200 else {
            throw BffError.notServed(status: answer.statusCode)
        }
        return data
    }
}
