...
/// The BFF as a series search sees it, with nothing behind it — one stub for the one protocol
/// searching for a series asks through, so the logo stub beside it is untouched by anything
/// that happens here.
///
...
@MainActor
final class StubSeries: SeriesSearching {
    ...
    /// The two asks share one gate, so they are told apart by what they are keyed under.
