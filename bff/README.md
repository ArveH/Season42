...
either's poster, which it keeps. What it keeps, it keeps for at most six months: the logo and
poster stores are swept on startup and every 24 hours, and neither serves an image that has aged
past the limit TMDB's terms set (ADR-0020).
...
not change under its own path (ADR-0008) — but it keeps nothing past 180 days: TMDB's terms forbid
caching what comes from them for longer than six months, so an aged logo is dropped and the next
ask fetches it again (ADR-0020).
...
included, until the file has aged past 180 days and is evicted on the same terms as a logo
(ADR-0020). The `series/` in the path is the other half of the key: TMDB numbers its series and its
